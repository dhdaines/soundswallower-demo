...
class DemoApp {
  ...
  async loadGrammar(name: string) {
    ...
    if (response.ok) {
      ...
      this.updateStatus(
        "Failed to fetch " + grammar_url + " :" + response.statusText
      );
    }
  }
  ...
  setupUI() {
    ...
    this.loadGrammar(
      this.selectTag.options[this.selectTag.selectedIndex].innerText
    );
  }

  async setupAudio() {
    this.updateStatus(
      "Initializing web audio, waiting for approval to access the microphone"
    );
  ...
  updateDownload() {
    let nsamp = 0;
    for (const block of this.speech) nsamp += block.length;
    const speech_buf = this.context.createBuffer(
      1,
      nsamp,
